...
export const Petmarketdetails = () => {
  const location = useLocation();
  const { items } = location.state || {};
  const { petMarketPrimaryDetail, serviceType, idProof } = items || {};
  ...
  const handleEditClinicHour = (service) => {
    console.log("Editing Clinic Hour:", service); // Debug log
  ...
  return (
    <Card border="light" className="bg-black shadow-sm mb-4">
      <Card.Body>
        ...
        {petMarketPrimaryDetail && (
          <Formik
            initialValues={petMarketPrimaryDetail}
            onSubmit={(values) => {
...
export default Petmarketdetails;
