...
export const Petmedicines = () => {
    ...
    const tabListStyles = {
      ...
      const navigateDetails = (items) => {
        console.log("itemsitemsitemsitemsitemsitems", items);
        history.push({
          pathname: "/Petmedicinesdetails",
          state: {items}
        });
      };
    const columns1 = useMemo(
      () => [
        ...
        { accessorKey: "bookingCancelDateTime", header: "bookingCancelDateTime" },
      ...
      const getVendorDetails = async () => {
        ...
        try {
          const requestBody = {
            ...
            radius: "3000000000",
            serviceId: "66a09abef47401a44145609a",
            find: { 
              serviceType: {
                $elemMatch: {
                  serviceId: "66a09abef47401a44145609a"
          ...
          const data = res.data?.data?.map((item) => {
            const petMarketPrimaryDetail = item.serviceType[0]?.petMarketPrimaryDetail || {};
            const businessName = item.serviceType[0]?.businessName || '';
      
            return {
              name: petMarketPrimaryDetail.name,
              mobile: petMarketPrimaryDetail.mobile,
              businessName: businessName,
              actions: (
                <div>
                  <FontAwesomeIcon
                    icon={faEye}
                    onClick={() => navigateDetails(item, petMarketPrimaryDetail, item.serviceType)}
                  />
                </div>
              ),
            };
            ...
            return {
              ...
              actions: (
                <div>
                  <FontAwesomeIcon
                    icon={faEye}
                    onClick={() => navigateDetails(item, personalDetails, item.serviceType)}
                  />
                </div>
              ),
            };
