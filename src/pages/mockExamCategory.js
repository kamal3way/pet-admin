...
export const MockExamCategory = () => {
  ...
  const navigateDetails = ( bankDetails,serviceType,service,homeWorkingHours ) => {
    console.log("homeWorkingHours", homeWorkingHours);
    history.push({
      pathname: "/PreparingFor",
      state: { bankDetails,serviceType ,service, homeWorkingHours }
    });
  };
  ...
  const handleServiceChange = async (event) => {
    ...
    try {
      const res = await axios.post(
        `https://pets.dev.savaapi.com/api/custom-api/dev1/clinic/get-vendor-with-single-service`,
        {
          find: { "serviceType.serviceId": selectedServiceId }
        },
        {
          headers: {
      ...
      const data = res.data?.data?.map((item) => ({
        pName: item.personalDetails.pName,
        pMobile: item.personalDetails.pMobile,
        ...
        actions: (
          <div>
            <FontAwesomeIcon
              icon={faEye}
              className="mx-3"
              onClick={() => navigateDetails(item.bankDetails,item.serviceType,item.service , item.homeWorkingHours)}
            />
          </div>
        ),
      }));
      const reversedData = data.reverse();
      setJsonData1(reversedData);
    } catch (error) {
      console.error(error);
    }
  };
        ...
        actions: (
          <div>
            <FontAwesomeIcon
              icon={faEye}
              className="mx-3"
              // onClick={() => navigateDetails(item.serviceLocation)}
            />
          </div>
        ),
      }));
