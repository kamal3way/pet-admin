        ...
        <Row className="d-block d-xl-flex align-items-center">
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            <div className="d-none d-sm-block">
              <h5>{category}</h5>
              <h3 className="mb-1">{title}</h3>
              {period && <h6 className="mb-1">Register Users:{period}</h6>}
              {percentage ? (
                <h6 className="mb-1">Active Users:{percentage}</h6>
              ) : (
                <></>
              )}
            </div>
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            {data.map((d) => (
              <h6
                ...
                className="fw-normal d-flex dkGray300"
              >
                <div className={d.className+" "}></div>
                {` ${d.label} `}: {`${d.value}`}
              </h6>
            ))}
          </Col>
...
export const RankingWidget = ({ groupName,startDate,endDate }) => {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v2/Subscription/getAllByPreparation/${groupName}?startDate=${startDate}&endDate=${endDate}`,
        {
  ...
  return (
    <Card border="light" className="shadow-sm" style={{ height: "11rem" }}>
      <Card.Body>
        {data.map((item, index) => (
